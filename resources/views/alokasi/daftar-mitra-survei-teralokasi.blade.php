            ...
            <div class="col-xl-12">
                <div class="card mb-4">
                    <div class="card-header">
                        ...
                        <strong class="">{{ isset(session()->get('sample')['nama_kegiatan_survei']) ? strtoupper(session()->get('sample')['nama_kegiatan_survei']) : '' }}</strong>
                        <button class="btn btn-sm btn-outline-primary btn-surtug float-right">Surat Tugas</button>
                        <button class="btn btn-sm btn-outline-primary btn-surtug float-right mr-2">SPK</button>
                        <button class="btn btn-sm btn-outline-primary btn-surtug float-right mr-2">BAST</button>
                    </div>
                    <div class="card-body">
                        <table id="mitra-survei-teralokasi" class="table table-striped table-bordered" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($list_mitra_terpilih as $mitra_survei_terpilih)
                                    <tr>
                                        <td>{{ $mitra_survei_terpilih->nama_mitra }}</td>
                                        <td>{{ $mitra_survei_terpilih->alamat_mitra }}</td>
                                        <td>{{ $mitra_survei_terpilih->kegiatan_survei }}</td>
                                        <td>{{ $mitra_survei_terpilih->idbs_teralokasi }}</td>
                                        <td>
                                            <button type="button"
                                            ...
                                            <a href="{{ url('/list_mitra_survei_teralokasi', $mitra_survei_terpilih->super_id) }}"
